...
export default function ChatInput({ onSend, disabled, mode, onModeChange, textareaRef: externalRef, isStreaming, onStop }: Props) {
  ...
  useEffect(() => {
    if (textareaRef.current) {
      ...
      // Cap height at 56px on mobile (~2 lines), 120px on desktop
      const maxH = typeof window !== "undefined" && window.innerWidth < 768 ? 56 : 120;
  ...
  return (
    <div className="space-y-1.5">
      ...
      {/* Input */}
      <div
        className="input-focus-ring flex items-end gap-3 rounded-2xl border px-4 py-2.5 sm:py-3"
        style={{
