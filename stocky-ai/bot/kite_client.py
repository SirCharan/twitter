...
from bot.kite_auth import get_authenticated_kite
...
class KiteClient:
    ...
    async def _get_kite(self) -> KiteConnect:
        ...
        if not kite:
            raise NotLoggedInError("Not logged in. Use /login or wait for auto-login.")
        return kite
    ...
    async def modify_order(self, order_id: str, variety: str = "regular", **kwargs) -> str:
        kite = await self._get_kite()
        return await self._run(kite.modify_order, variety, order_id, **kwargs)
